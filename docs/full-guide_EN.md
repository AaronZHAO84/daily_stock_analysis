...
| `REPORT_TYPE` | Report type: `simple` (concise), `full` (complete), `brief` (3-5 sentences), `summary_lite` (summary + per-stock brief), Docker recommended: `summary_lite` | Optional |
...
| `SEARXNG_PUBLIC_INSTANCES_ENABLED` | Auto-discover public SearXNG instances from `searx.space` when `SEARXNG_BASE_URLS` is empty (default `false`; public instances may rate-limit) | Optional |
...
| `REALTIME_SOURCE_PRIORITY` | Real-time quote source priority (comma-separated), default `efinance,akshare_sina,akshare_em,tencent`; add `tickflow` explicitly to use TickFlow realtime quotes | See .env.example | Optional |
