...
| `REPORT_TYPE` | 报告类型：`simple`(精简)、`full`(完整)、`brief`(3-5句概括)、`summary_lite`(整体摘要+个股速览)，Docker环境推荐设为 `summary_lite` | 可选 |
...
| `SEARXNG_PUBLIC_INSTANCES_ENABLED` | 是否在 `SEARXNG_BASE_URLS` 为空时自动从 `searx.space` 获取公共实例（默认 `false`；公共实例可能限流） | 可选 |
...
| `REALTIME_SOURCE_PRIORITY` | 实时行情源优先级，默认 `efinance,akshare_sina,akshare_em,tencent`；需要显式加入 `tickflow` 才会使用 TickFlow 实时行情。 | 见 `.env.example` | 可选 |
